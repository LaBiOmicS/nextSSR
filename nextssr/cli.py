...
@click.option('--config', '-c', type=click.Path(exists=True), help='Path to nextSSR configuration file (nextssr.yaml / nextssr.json)')
@click.option('--output-dir', '-o', default='results', help='Output directory for artifacts')
@click.option('--threads', '-t', default=None, type=int, help='Number of parallel CPU worker threads/processes')
@click.option('--design-primers/--no-primers', default=True, help='Automatically design PCR primers for identified SSRs')
@click.option('--flank-len', default=150, help='Flanking sequence window size in bp (default: 150bp)')
@click.option('--opt-tm', default=58.0, help='Optimal primer melting temperature Tm in °C')
@click.option('--min-product-size', default=100, help='Minimum amplicon product size in bp')
@click.option('--max-product-size', default=300, help='Maximum amplicon product size in bp')
@click.option('--gpu/--no-gpu', default=False, help='Enable GPU hardware acceleration if CUDA is available')
@click.option('--ro-crate/--no-ro-crate', default=True, help='Generate FAIR RO-Crate JSON-LD metadata')
def run_analysis(fasta_file: str, config: str, output_dir: str, threads: int, design_primers: bool, flank_len: int, opt_tm: float, min_product_size: int, max_product_size: int, gpu: bool, ro_crate: bool):
    ...
    param_table.add_row("[bold]Hardware Acceleration:[/bold]", "[green]GPU (CUDA Enabled)[/green]" if cfg.use_gpu else "CPU Vectorized")
    param_table.add_row("[bold]Primer Design Engine:[/bold]", f"[green]Enabled[/green] (Tm: {cfg.opt_tm}°C | Amplicons: {cfg.min_product_size}-{cfg.max_product_size}bp)" if cfg.design_primers else "[yellow]Disabled[/yellow]")
    param_table.add_row("[bold]FAIR RO-Crate Provenance:[/bold]", "[green]Enabled[/green]" if cfg.generate_ro_crate else "[yellow]Disabled[/yellow]")

    console.print(Panel(param_table, title="[bold cyan]Run Parameters[/bold cyan]", border_style="dim"))
    ...
    ) if cfg.design_primers else None
    ...
    with Progress(
        ...
        for res in finder.analyze_batch_parallel(fasta_stream):
            ...
            if primer_designer:
                for ssr in res.ssrs:
                    pair = primer_designer.design_primers(ssr.flank_5p, ssr.sequence, ssr.flank_3p)
            ...
            progress.update(task, description=f"[cyan]Analyzed {len(results)} sequences ({total_ssrs} SSRs found)...")
    ...
    summary_table.add_row("Valid PCR Primer Pairs Designed", f"{total_primers_designed} / {total_ssrs}")
    ...
    art_table.add_row("[bold]GFF3 Sequence Ontology Annotation:[/bold]", artifacts["gff"])
    ...
    console.print(Panel(art_table, title="[bold green]Generated Run Artifacts[/bold green]", border_style="green"))


@main.command(name="init-config")
@click.option('--output', '-o', default='nextssr.yaml', help='Output configuration file path')
def init_config(output: str):
    """Generate a documented default nextSSR YAML configuration file."""
    config_path = SSRConfig.generate_default_config(output)
    console.print(f"[green]✓ Created default nextSSR configuration file at:[/green] [bold]{config_path}[/bold]")
