...
class PrimerDesigner:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                "primer3-py not installed. Using native nextSSR thermodynamic primer engine."
            )
    ...
    def calculate_tm(seq: str) -> float:
        ...
        if len(seq) < 14:
            w_tm = (sum(1 for b in seq if b in ("A", "T")) * 2) + (
                sum(1 for b in seq if b in ("G", "C")) * 4
            )
            return float(w_tm)
    ...
    def design_primers(
        ...
        if "N" in full_seq:
            return PrimerPair(
                ...
                product_size=0,
                status="REJECTED",
                error_reason="Contains ambiguous N bases",
            )
        ...
        if len(flank_5p) < self.min_size or len(flank_3p) < self.min_size:
            return PrimerPair(
                ...
                product_size=0,
                status="REJECTED",
                error_reason="Flanking regions too short",
            )
        ...
        for length in range(self.min_size, self.max_size + 1):
            for i in range(0, target_start_idx - length + 1):
                ...
                if (
                    self.min_tm <= tm <= self.max_tm
                    and self.min_gc <= gc <= self.max_gc
                ):
        ...
        if not candidates_f or not candidates_r:
            return PrimerPair(
                ...
                product_size=0,
                status="REJECTED",
                error_reason="No primers met Tm/GC constraints",
            )
        ...
        return PrimerPair(
            ...
            product_size=0,
            status="REJECTED",
            error_reason="No pair met product size or Tm diff limits",
        )
