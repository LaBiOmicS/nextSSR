...
def _get_perfect_patterns(motif_len: int, min_reps: int) -> Tuple[re.Pattern, List[re.Pattern]]:
    ...
    if key not in _PERFECT_PATTERNS:
        ...
        for sub_len in range(1, motif_len):
            if motif_len % sub_len == 0:
                sub_p_list.append(re.compile(f"^([ACGT]{{{sub_len}}})\\1{{{motif_len // sub_len - 1}}}$"))
        _PERFECT_PATTERNS[key] = (p, sub_p_list)
    return _PERFECT_PATTERNS[key]

def _analyze_single_sequence_worker(args: Tuple[str, str, dict, int]) -> SequenceAnalysisResult:
...
class SSRFinder:
    ...
    def analyze_sequence(self, seq_id: str, sequence: str) -> SequenceAnalysisResult:
        ...
        return _analyze_single_sequence_worker((seq_id, sequence, self.config.unit_min_repeats, self.flank_len))

    def analyze_batch_parallel(self, sequence_generator: Generator[Tuple[str, str], None, None]) -> Generator[SequenceAnalysisResult, None, None]:
        ...
        if len(first_batch) == 1 and not hasattr(sequence_generator, '__next__'):
            ...
            yield _analyze_single_sequence_worker((seq_id, seq_str, self.config.unit_min_repeats, self.flank_len))
            return
        ...
        if self.config.threads == 1:
            ...
            with ProcessPoolExecutor(max_workers=self.config.threads) as executor:
                batch = [(seq_id, seq_str, self.config.unit_min_repeats, self.flank_len) for seq_id, seq_str in first_batch]
                results = executor.map(_analyze_single_sequence_worker, batch, chunksize=max(1, len(batch) // self.config.threads))
                for res in results:
                    yield res

                batch.clear()
                for seq_id, seq_str in sequence_generator:
                    batch.append((seq_id, seq_str, self.config.unit_min_repeats, self.flank_len))
                ...
                if batch:
                    results = executor.map(_analyze_single_sequence_worker, batch, chunksize=1)
                    for res in results:
                        yield res
